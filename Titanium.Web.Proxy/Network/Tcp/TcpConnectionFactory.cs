    ...
    internal class TcpConnectionFactory : IDisposable
    {
            ...
            bool noCache, CancellationToken cancellationToken)
        {
            ...
            if (proxyServer.EnableConnectionPool && !noCache)
            {
                ...
                if (cache.TryGetValue(cacheKey, out existingConnections))
                {
                    while (existingConnections.Count > 0)
                    {
                        TcpServerConnection recentConnection;
                        if (existingConnections.TryDequeue(out recentConnection))
                        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (useUpstreamProxy && (isConnect || isHttps))
                {
                    ...
                    Version _;
                    Response.ParseResponseLine(httpStatus, out _, out statusCode, out statusDescription);
            ...
            while (runCleanUpTask)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            ConcurrentQueue<TcpServerConnection> _;
